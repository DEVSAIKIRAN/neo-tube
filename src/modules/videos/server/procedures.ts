    ...
    getOne:baseProcedure.input(z.object({id: z.string().uuid()})).query(async ({ ctx, input }) => {
        ...
        const [existingVideo] = await db.with(viewerReactions , viewerSubscriptions).select({
            ...
            user: {
                ...
                isSubscribed: isNotNull(viewerSubscriptions.viewerId).mapWith(Boolean)
            },
            viewCount: db.$count(videoViews, eq(videoViews.videoId, videos.id)),
            likeCount: db.$count(videoReactions, eq(videoReactions.type, "like")),
            dislikeCount: db.$count(videoReactions, eq(videoReactions.type, "dislike")),
            viewerReaction: viewerReactions.type,
        }).from(videos).innerJoin(users, eq(videos.userId, users.id)).leftJoin(viewerReactions, eq(videoReactions.videoId, videos.id)).leftJoin(viewerSubscriptions,eq(viewerSubscriptions.viewerId , user.id) ).where(eq(videos.id, input.id))//.groupBy(
       //  videos.id,
         //   users.id,
    ...
    update: protectedProcedure.input(videoUpdateSchema).mutation(async ({ctx, input})=> {
      ...
      const [updatedVideo] =  await db.update(videos).set({
        ...
        updatedAt: new Date()
         ...
         eq(videos.id, input.id),
         eq(videos.userId, userId))).returning();
